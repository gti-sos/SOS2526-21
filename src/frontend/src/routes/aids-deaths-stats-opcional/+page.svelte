<script>
	...
	// Reemplaza la función login con esta versión más limpia:
	...
	async function deleteDato(codecountry, year) {
		if (confirm('¿Estás seguro de que quieres borrar este elemento?')) {
			...
			const res = await fetch(BASE_API + `/${codecountry}/${year}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }});
	...
	async function deleteDatos() {
		if (confirm('¿Estás seguro de que quieres borrar toda la base de datos?')) {
			...
			const res = await fetch(BASE_API, { method: 'DELETE',  headers: { 'Authorization': `Bearer ${token}` } });
			result = await res.status;
			if (res.status == 200) await getDatos();
		}
	}
	...
	<section class="card">
		{#if !loggedIn}
			...
			<div class="filtros-grid">
				<div class="field">
					<label>Usuario</label>
					<input type="text" bind:value={loginUsername} placeholder="admin" />
				</div>
				<div class="field">
